...
while True:
    ...
    if k==27:    # Esc key to stop
        ...
        while(True):
            # Motion info
            motionSel = moveSet[moveIndex]
            moveIndex += 1
            if moveIndex > 1: moveIndex = 0
            if motionSel in 'fb': motionDur = randint(150, 1000)
            else:  motionDur = randint(150, 750)

            print(f"\n\nMoving {motionSel} for {motionDur}\n")
            roverComms.doMotion(motionSel, motionDur)
            time.sleep(motionDur/1000+0.5)
            ...
            # Pickle LED Data Dictionary
            outFile = open("data/"+fileName, 'wb')
            outDict['motionSel'] = motionSel
            outDict['motionDur'] = motionDur
