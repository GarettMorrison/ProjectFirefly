...
while True:
    ...
    if len(sorted_contours) > 0:        
        ...
        print(f"row:{round(rowPos,2)}   col:{round(heightPos,2)}   img_x:{round(imgPos_x,2)}   img_y:{round(imgPos_y,2)}   img_s:{round(imgPos_s,2)}")
    ...
    cv2.imshow('Display', imgSet) # Show original) # Show original
    cv2.waitKey(1)
...
outFile = open('py/outData.pkl', 'wb')
